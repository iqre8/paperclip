...
const KIMI_AUTH_REQUIRED_RE = /(?:not\s+logged\s+in|please\s+log\s+in|please\s+run\s+`?kimi\s+login`?|login\s+required|requires\s+login|unauthorized|authentication\s+required)/i;
...
export function extractKimiLoginUrl(text: string): string | null {
  const match = text.match(URL_RE);
  if (!match || match.length === 0) return null;
  for (const rawUrl of match) {
    const cleaned = rawUrl.replace(/[\])}.!,?;:'"]+$/g, "");
    if (cleaned.includes("kimi") || cleaned.includes("moonshot") || cleaned.includes("auth")) {
      return cleaned;
    }
  }
...
export function detectKimiLoginRequired(input: {
  ...
  return {
    requiresLogin,
    loginUrl: extractKimiLoginUrl([input.stdout, input.stderr].join("\n")),
  };
}
...
export function isKimiMaxStepsResult(parsed: Record<string, unknown> | null | undefined): boolean {
  ...
  if (subtype === "error_max_steps") return true;

  const stopReason = asString(parsed.stop_reason, "").trim().toLowerCase();
  if (stopReason === "max_steps") return true;
