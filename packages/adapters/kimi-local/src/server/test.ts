...
export async function testEnvironment(
  ...
  if (isNonEmpty(configApiKey) || isNonEmpty(hostApiKey)) {
    ...
    checks.push({
      code: "kimi_api_key_overrides_subscription",
      level: "warn",
      message:
        "KIMI_API_KEY is set. Kimi will use API-key auth instead of subscription credentials.",
      detail: `Detected in ${source}.`,
      hint: "Unset KIMI_API_KEY if you want subscription-based Kimi login behavior.",
    });
  } else {
    checks.push({
      code: "kimi_subscription_mode_possible",
      level: "info",
      message: "KIMI_API_KEY is not set; subscription-based auth can be used if Kimi is logged in.",
    });
  }
      ...
      if (probe.timedOut) {
        ...
        checks.push({
          ...
          ...(hasHello
            ? {}
            : {
                hint: "Try the probe manually (`kimi --print --output-format stream-json --verbose`) and prompt `Respond with hello`.",
              }),
        });
      } else {
        checks.push({
          ...
          ...(detail ? { detail } : {}),
          hint: "Run `kimi --print --output-format stream-json --verbose` manually in this directory and prompt `Respond with hello` to debug.",
        });
      }
    }
  }
