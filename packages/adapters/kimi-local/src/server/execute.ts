...
/**
 * Create a tmpdir with `.kimi/skills/` containing symlinks to skills from
 * the repo's `skills/` directory, so `--skills-dir` makes Kimi CLI discover
 * them as proper registered skills.
 */
async function buildSkillsDir(): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-kimi-skills-"));
  const target = path.join(tmp, ".kimi", "skills");
...
function resolveKimiBillingType(env: Record<string, string>): "api" | "subscription" {
  // Kimi uses API-key auth when KIMI_API_KEY is present; otherwise rely on local login/session auth.
  return hasNonEmptyEnvValue(env, "KIMI_API_KEY") ? "api" : "subscription";
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const maxSteps = asNumber(config.maxStepsPerRun, 0);
  const instructionsFilePath = asString(config.instructionsFilePath, "").trim();
  const instructionsFileDir = instructionsFilePath ? `${path.dirname(instructionsFilePath)}/` : "";
  const commandNotes = instructionsFilePath
    ? [
        `Injected agent instructions via --agent-file ${instructionsFilePath} (with path directive appended)`,
      ]
    : [];
  ...
  const billingType = resolveKimiBillingType(env);
  ...
  const buildKimiArgs = (resumeSessionId: string | null) => {
    ...
    if (effectiveInstructionsFilePath) {
      args.push("--agent-file", effectiveInstructionsFilePath);
    }
  ...
  const toAdapterResult = (
    ...
    const resolvedSessionParams = resolvedSessionId
      ...
      : null;
    const clearSessionForMaxSteps = isKimiMaxStepsResult(parsed);
